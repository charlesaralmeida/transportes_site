...
const CidadeByUFSelector = (props) => {
    ...
    return (<div style={style}>
        <UFs
            setCidadesUF={setCidadesUF}
            changeUF={handleUFChange}
            changeCidade={handleCidadeChange}
...
CidadeByUFSelector.propTypes = {
    getCidade: PropTypes.func.isRequired,
    getUF: PropTypes.func.isRequired,
}

const UFs = (props) => {
    const uflist = ['AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'MG', 'PA',
        'PB', 'PR', 'PE', 'PI', 'RJ', 'RN', 'RS', 'RO', 'RR', 'SC', 'SP', 'SE', 'TO'];
    const UF_API_URL = 'https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome';

    const MSG = {
        ERROR_BUSCAR_CIDADE: 'Erro ao buscar cidades',
    }
    
    const getUFID = (ufValue) => {
        return fetch(UF_API_URL)
            .then(response => response.json())
            .then(response => {
                if (response.length) {
                    let ufTarget = response.filter(uf => uf['sigla'] === ufValue);
                    return ufTarget[0]['id'];
                } else
                    return [];
            })
    ...
    const getCidadesUF = (UFID) => {
        const CIDADES_API_URL = `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${UFID}/municipios?orderBy=nome`;
        return fetch(CIDADES_API_URL)
            .then(response => response.json())
            .then(response => {
                if (response.length)
                    return response.map(cidade => cidade['nome'])
                else
                    return [];
            })
            .catch(() => props.changeCidade(MSG.ERROR_BUSCAR_CIDADE))
    }
                ...
                getCidadesUF(UFID)
                    .then(listaCidades => {                              
                        if (listaCidades.length)
                            props.setCidadesUF(listaCidades)
                        else {
                            props.setCidadesUF([MSG.ERROR_BUSCAR_CIDADE])
                            props.changeCidade(MSG.ERROR_BUSCAR_CIDADE);
                        }
                    })
                    .then(() => props.changeUF(value))
                    .then(() => props.changeCidade(''))
                    .catch(error => alert(error))
            })
    ...
    return (<FormControl style={style}>
        ...
        <Select
            ...
            value={props.UFSelected}
            onChange={e => handleChange(e.target.value)}
            label='UF'
        >
            {uflist.map((uf, index) => <MenuItem value={uf} key={index}>{uf}</MenuItem>)}
        </Select>
        <div id='cidades'></div>
    </FormControl>)
}
...
const Cidades = (props) => {    
    ...
    return (<FormControl style={style}>
        ...
        <Select
            ...
            value={props.cidadeSelected}
            onChange={e => handleChange(e.target.value)}
            label='Cidade'
        >
            {props.cidadesList && props.cidadesList.map((cidade, index) => <MenuItem value={cidade} key={index}>{cidade}</MenuItem>)}
        </Select>
    </FormControl>)
}
...
export default CidadeByUFSelector;
